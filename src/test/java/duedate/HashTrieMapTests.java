...
public class HashTrieMapTests {
    ...
    @Test()
    @Timeout(value = 3000, unit = TimeUnit.MILLISECONDS)
    public void singleChar_Paths() {
        HashTrieMap<Character, AlphabeticString, String> STUDENT_TRIE = new HashTrieMap<>(AlphabeticString.class);

        String testey = STUDENT_TRIE.find(toAlphabeticString(""));


        STUDENT_TRIE.insert(toAlphabeticString(""), "Root");

        STUDENT_TRIE.delete(toAlphabeticString(""));

        STUDENT_TRIE.insert(toAlphabeticString("a"), "Value A");
        String test = STUDENT_TRIE.find(toAlphabeticString(""));
        test = STUDENT_TRIE.find(toAlphabeticString("a"));
        STUDENT_TRIE.delete(toAlphabeticString(""));
        test = STUDENT_TRIE.find(toAlphabeticString(""));
        STUDENT_TRIE.delete(toAlphabeticString("a"));
        test = STUDENT_TRIE.find(toAlphabeticString("a"));
       ...
       boolean tester = STUDENT_TRIE.findPrefix(toAlphabeticString(""));


       STUDENT_TRIE.clear();

        String[] words = {"dog", "doggy", "doge", "dragon", "cat", "draggin", "", "hi", "heyo", "peace"};
        addAll(STUDENT_TRIE, words);

        assertEquals(10, STUDENT_TRIE.size());
        STUDENT_TRIE.delete(toAlphabeticString("doge"));
        STUDENT_TRIE.findPrefix(toAlphabeticString("do"));
        STUDENT_TRIE.delete(toAlphabeticString(""));
        STUDENT_TRIE.delete(toAlphabeticString("hi"));
        STUDENT_TRIE.delete(toAlphabeticString("cat"));
        STUDENT_TRIE.findPrefix(toAlphabeticString("dog"));
        STUDENT_TRIE.findPrefix(toAlphabeticString("do"));
        STUDENT_TRIE.delete(toAlphabeticString(""));
        assertEquals(6, STUDENT_TRIE.size());
        assertEquals(null, STUDENT_TRIE.find(toAlphabeticString("dragony")));

        STUDENT_TRIE.findPrefix(toAlphabeticString(""));
        String temp = STUDENT_TRIE.insert(toAlphabeticString("dragon"), "tester");
        assertEquals("DRAGON", temp);
        String temper = STUDENT_TRIE.insert(toAlphabeticString(""), "testing");
        assertEquals(null, temper);



        int val = STUDENT_TRIE.size();
        STUDENT_TRIE.delete(toAlphabeticString(""));
        val = STUDENT_TRIE.size();
        STUDENT_TRIE.delete(toAlphabeticString("Samantha"));
        STUDENT_TRIE.delete(toAlphabeticString("Damien"));
        STUDENT_TRIE.delete(toAlphabeticString("Richard"));

        STUDENT_TRIE.delete(toAlphabeticString("d"));
